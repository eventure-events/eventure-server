...
eventRouter.get('/event', (req, res, next) => {
  ...
  Eventure.find().then((all) => {
    if (!all) return httpError(404, 'No events found.');
    res.json(all);
  }).catch(next);
});
...
eventRouter.get('/event/:id', (req, res, next) => {
  ...
  Eventure.findById({'_id': req.params.id})
    .then((evnt) => {
      if (!evnt) return httpError(404, 'No event found.');
      res.json(evnt);
    }).catch(next);
});
...
eventRouter.put('/event/:id', jsonParser, (req, res, next) => {
  ...
  Eventure.findByIdAndUpdate({'_id': req.params.id}, req.body, {new: true})
    .then((evnt) => {
      if (!evnt) return httpError(404, 'No event found.');
      res.json(evnt);
    }).catch(next);
...
eventRouter.delete('/event/:id', jsonParser, (req, res, next) => {
  ...
  Eventure.findByIdAndRemove({'_id': req.params.id})
    .then((evnt) => {
      if (!evnt) return httpError(404, 'No event found.');
      res.json(evnt);
    }).catch(next);
